log.info("[monster_hp_to_json.lua] loaded")
        ...
        if enemyman then
            for i = 0, 4 do
                ...
                if enemy then
                    local hp_entry = hp_table[enemy]
                    if hp_entry then
                        table.insert(monster_data.monsters, {
                            name = hp_entry.name,
                            hp = math.floor(hp_entry.hp),
                            max_hp = math.floor(hp_entry.max_hp),
                            hp_percent = math.floor((hp_entry.hp / hp_entry.max_hp) * 100)
                        })
                    end
                end
            end
            
            json.dump_file("monster_hp_data.json", monster_data)
        end
    end
)
